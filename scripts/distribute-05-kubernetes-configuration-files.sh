...
distribute_worker_kubeconfigs() {
  for instance in worker-0 worker-1 worker-2; do
    local worker_node_files="${instance}.kubeconfig kube-proxy.kubeconfig"
    scp_to_node ${instance} "${worker_node_files}"
  done
}

distribute_controller_kubeconfigs() {
  for instance in controller-0 controller-1 controller-2; do
    local controller_files="admin.kubeconfig kube-controller-manager.kubeconfig kube-scheduler.kubeconfig"
